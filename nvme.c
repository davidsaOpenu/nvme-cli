...
#define NVME_OBJ_ID_MAXLEN 64 /* contract with driver - max length of object id */
...
static int handle_obj_op(int argc, char **argv, int opcode, void *cfg, char* name)
{
	...
	switch(opcode)
	{
		case nvme_cmd_obj_write:
			if(argc < 5)
			{
				fprintf(stderr, "Invalid arguments, object write usage: nvme objw <device> <obj_id> <--data=file-path> <--data-size=x(in bytes)>\n");
				return EINVAL;
			}
			...
			strncpy(name, argv[4], NVME_OBJ_ID_MAXLEN);
			break;
		case nvme_cmd_obj_read:
			if(argc < 5)
			{
				fprintf(stderr, "Invalid arguments, object read usage: nvme objr <device> <obj_id> <--data=file-path> <--data-size=x(in bytes)>\n");
				return EINVAL;
			}
			strncpy(name, argv[4], NVME_OBJ_ID_MAXLEN);
			break;
		case nvme_cmd_obj_delete:
			if(argc < 3)
			{
				fprintf(stderr, "Invalid arguments, object delete usage: nvme objd <device> <obj_id>\n");
				return EINVAL;
			}
			strncpy(name, argv[2], NVME_OBJ_ID_MAXLEN);
			break;
		case nvme_cmd_obj_list:
			if(argc < 4)
			{
				fprintf(stderr, "Invalid arguments, object list usage: nvme objl <device> <--data=file-path> <--data-size=x(in bytes)>\n");
				return EINVAL;
			}
	}
	return 0;
		     ...
		     int argc, char **argv)
{
	...
	gettimeofday(&start_time, NULL);
	if((err = nvme_obj_io(fd, opcode, cfg.offset, buffer_size, &buffer, (__u8*)name)))
	{
		fprintf(stderr, "ioctl call failed with errno: %d\n",errno);
		goto free_and_return;
	}
	gettimeofday(&end_time, NULL);
	if (cfg.latency)
