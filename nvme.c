...
#define OBJ_REALLOC(x) !(x & 1 || x & 2) /* used for objl and objr - used to signify the need to reallocate buffer if it is insufficient */
		     ...
		     int argc, char **argv)
{
	...
	gettimeofday(&start_time, NULL);
	if((err = nvme_obj_io(fd, opcode, cfg.start_block, buffer_size, &buffer, OBJ_REALLOC(opcode), (__u8*)name)))
	{
