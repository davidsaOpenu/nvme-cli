...
static int handle_obj_op(int argc, char **argv, int opcode, void *io, char* name)
{
	...
	char hex_key[NVME_OBJ_ID_MAXLEN + 1] = {0};
	__u64 hi, lo;
	ptr->opcode = opcode;
	switch(opcode) {
		case nvme_kv_store:
		...
		case nvme_kv_list:
			...
			ptr->key_len = 0;
			break;
		case nvme_kv_exist: /* parses hex string into 2 64 bit integers */
			len = strlen(argv[argc - 1]);
			hi = 0;
			if (len > NVME_OBJ_ID_MAXLEN) {
				strncpy(hex_key, argv[argc - 1], NVME_OBJ_ID_MAXLEN);
				hex_key[len - NVME_OBJ_ID_MAXLEN] = '\0';
				sscanf(hex_key, "%"PRIx64, (long unsigned*)&lo);
				strncpy(hex_key, argv[argc - 1] + (len - NVME_OBJ_ID_MAXLEN), NVME_OBJ_ID_MAXLEN);
			}
			else strncpy(hex_key, argv[argc - 1], NVME_OBJ_ID_MAXLEN);
			sscanf(hex_key, "%"PRIx64, (long unsigned*)&hi);
			ptr->key_high = hi;
			ptr->key_low = lo;
			ptr->key_len = 16;
			ptr->length = 0;
...
static int obj_read_parse_list(int fd, int dfd, void *io, __u32 *length, void **buffer)
{
	...
	__u32 buffer_size = *length, count = 0;
	char out_string[NVME_OBJ_ID_MAXLEN + 1] = {0};
		...
		*length = buffer_size;
		...
		for(i = 0; i < count && err >= 0; i++) { /* parse kv list format */
			...
			if (skip)
				...
				lo = *(__u64*) ptr;
				hi = *(__u64*) (ptr + 8);
				err = sprintf(out_string, "%"PRIx64, (unsigned long)lo);
				fprintf(out, "%s", out_string);
				err = sprintf(out_string, "%"PRIx64, (unsigned long)hi);
				fprintf(out, "%.*s", NVME_OBJ_ID_MAXLEN - err, ZERO_STRING); /* padding zeroes */
				fprintf(out, "%s\n", out_string);
			}
