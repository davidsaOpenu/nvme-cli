...
static int obj_write_back(int fd, void *buffer, long long buffer_size, int opcode)
{
	char *temp_buf = NULL, *ptr1 = NULL, *ptr2 = NULL;
	char size = sizeof(__u64);
	long long temp_buf_size = 0;
	int err = 0;
	if (opcode == nvme_cmd_obj_list) {
		temp_buf_size = buffer_size + /* for each object - add mem for ' ' and '\n' */ (buffer_size)/(size);
		temp_buf = (char*)calloc(temp_buf_size, sizeof(char));
		if (!temp_buf) {
			free(temp_buf);
			return ENOMEM;
		}
		ptr1 = (char*)buffer;
		ptr2 = temp_buf;
		while(ptr1 - (char*)buffer < buffer_size)
		{
			memcpy(ptr2 , ptr1, size);
			ptr1 += size;
			ptr2 += size;
			*ptr2 = ' ';
			memcpy(ptr2 + 1, ptr1, size /* object size is represented using 64 bit integer */);
			*(ptr2 + 9) = '\n';
			ptr1 += size;
			ptr2 += size + 2;
		}
		err = write(fd, temp_buf, temp_buf_size);
		free(temp_buf);
		return err;
	}
	else
