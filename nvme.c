...
static int obj_write_back(int fd, void *buffer, __u64 buffer_size, int opcode)
{
	if (opcode == nvme_kv_list) {
		...
		for(i = 0; i < count && err >= 0; i++) {
			...
			err += fprintf(out, "%.16s\n", ptr);
			fprintf(stderr, "%.16s\n", ptr);
			ptr = (char*)((((__u64)ptr + key_len + 3) / 4) * 4);
		}
		return err;
	}
	else
		     ...
		     int argc, char **argv)
{
	...
	/* TODO: Remove this once offset support is added back */
	io.length = 1024;
	buffer_size = 1024;
	if (io.length && 
	...
	while (!(opcode & 1) && !err && buffer_size) {
		...
		io.offset += length;
		if (1/*length < buffer_size*/) break; /* TODO: Return regular break condition once offset support is added back */
	}
	if (opcode & 1)
	{
